...
export const useLatestReleases = (minCount: number = 4, maxCount: number = 8) => {
    const [latestProducts, setLatestProducts] = useState<Product[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);
    
    const { products, getProducts, loading: productsLoading, error: productsError } = useProducts();

    // Função para selecionar e embaralhar parcialmente os lançamentos
    const getLatestProducts = useCallback((productList: Product[], count: number): Product[] => {
