  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (formValues.salePrice) {
      ...
      if (isNaN(salePriceValue))
        errors.salePrice = "Preço promocional inválido";
    ...
    if (formValues.sizes && formValues.sizes.length > 0) {
      formValues.sizes.forEach((size, index) => {
        ...
        if (Number(size.stock) < 0)
          errors[`sizes-stock-${index}`] = "Estoque não pode ser negativo";
      });
    }
    ...
    if (formValues.colors && formValues.colors.length > 0) {
      formValues.colors.forEach((color, index) => {
        if (!color.colorName)
          errors[`colors-${index}`] = "Nome da cor é obrigatório";
        if (Number(color.stock) < 0)
          errors[`colors-stock-${index}`] = "Estoque não pode ser negativo";
      });
    }
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!ALLOWED_FILE_TYPES.includes(file.type)) {
        setFormErrors((prev) => ({
          ...prev,
          image: "Tipo de arquivo não permitido",
        }));
        return;
      }
      if (file.size > MAX_FILE_SIZE) {
        setFormErrors((prev) => ({
          ...prev,
          image: "Imagem muito grande (máx. 5MB)",
        }));
        return;
      }
  ...
  const handleAdditionalImagesChange = (
    ...
    if (e.target.files) {
      ...
      const validFiles = files.filter((file) => {
        ...
        if (!isValidType) {
          toast.error(
            `Arquivo "${file.name}": Apenas imagens JPEG, PNG ou WEBP são permitidas`
          );
        }
        if (!isValidSize) {
      ...
      if (
        validFiles.length + (formValues.additionalImages?.length || 0) >
        MAX_ADDITIONAL_IMAGES
      ) {
        toast.error(
          `Máximo de ${MAX_ADDITIONAL_IMAGES} imagens adicionais permitidas`
        );
        return;
      }
  ...
  const addSize = () => {
    if (newSize.size.trim()) {
      setFormValues((prev) => ({
        ...
        sizes: [
          ...(prev.sizes || []),
          { ...newSize, stock: Number.parseInt(newSize.stock.toString()) || 0 },
        ],
      }));
                  ...
                  >
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2 pt-4">
                        ...
                        {formValues.additionalImages &&
                          formValues.additionalImages.length > 0 && (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4">
                              <AnimatePresence>
                                {formValues.additionalImages.map(
                                  (img, index) => (
                                    <motion.div
                                      key={index}
                                ...
                                <div className="flex items-center gap-2">
                                  <div className="w-10 h-10 rounded-lg overflow-hidden border border-gray-200 flex items-center justify-center">
                                    <ColorPicker
                                      ...
                                      onChange={(value) =>
                                        updateColor(index, "colorCode", value)
                                      }
                                    />
                                  </div>
                                  <div
